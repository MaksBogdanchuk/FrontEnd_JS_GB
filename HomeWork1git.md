...
Наименование ветки можно изменить с помощью комманды `git branch -m (NewNameBranch)`..

Для конфликта создам ветку `conflict`.
X=20, Y=20, Sum=X+Y=40.

Конфликт был исчерпан, можно слить ветку с веткой `master`.
Для слияния необходимо прописать `git merge + (наименование ветки с которой будут браться изменения)`.

### IV. Работа с удаленными репозиториями.

- Копировать внешний репозиторий на свой ПК можно командой `git clone`

> далее к команде `git clone` подписывается адрес(например копируем ссылку на репозиторий с GitHub), откуда мы берем внешний репозиторий.

- Также возможно скачать внешний репозиторий и слить его с нашей версией, для этого пишем `git pull`.
- Чтобы залить на внешний репозиторий нашу версию репозитория необходимо ввести `git push`.
